...
class OrderFlowTest extends BaseIntegrationTest {
    ...
    void tc01_completeOrderFlow() throws Exception {
        ...
        // 接单后状态应为 PENDING_ACCEPT（异步事件推进，用轮询等待避免 flaky）
        testHelper.waitForOrderStatus(volToken, orderId, OrderStatus.PENDING_ACCEPT, 3);

        // 志愿者依次出发 → 到达 → 确认开始服务，才进入 IN_PROGRESS
        testHelper.driverEnRoute(volToken, orderId);
        testHelper.driverArrived(volToken, orderId);
        testHelper.startService(volToken, orderId);
