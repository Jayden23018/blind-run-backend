...
-- 15. 走散检测（陪跑中距离过大）→ 通知盲人
INSERT IGNORE INTO notification_templates (event_type, target_role, channel, template_text, tts_text, priority, is_active)
VALUES ('ESCORT_DISTANCE_ALERT', 'BLIND_USER', 'WEBSOCKET', '与志愿者的距离似乎有点远', '你和志愿者的距离似乎有点远，请留在原地，志愿者正在确认位置', 'HIGH', true);

-- 16. 走散检测（陪跑中距离过大）→ 通知志愿者
INSERT IGNORE INTO notification_templates (event_type, target_role, channel, template_text, tts_text, priority, is_active)
VALUES ('ESCORT_DISTANCE_ALERT', 'VOLUNTEER', 'WEBSOCKET', '与盲人用户的距离似乎有点远', '你和盲人用户的距离似乎有点远，请尽快确认对方位置', 'HIGH', true);

-- 17. 走散检测（陪跑中信号缺失）→ 通知盲人
INSERT IGNORE INTO notification_templates (event_type, target_role, channel, template_text, tts_text, priority, is_active)
VALUES ('ESCORT_SIGNAL_LOST', 'BLIND_USER', 'WEBSOCKET', '暂时无法获取对方位置，正在为你确认安全', '暂时无法获取志愿者位置，请留在原地，我们正在为你确认安全', 'HIGH', true);

-- 18. 走散检测（陪跑中信号缺失）→ 通知志愿者
INSERT IGNORE INTO notification_templates (event_type, target_role, channel, template_text, tts_text, priority, is_active)
VALUES ('ESCORT_SIGNAL_LOST', 'VOLUNTEER', 'WEBSOCKET', '暂时无法获取对方位置，正在为你确认安全', '暂时无法获取盲人用户位置，请尽快确认对方安全', 'HIGH', true);
...
-- 3. 紧急情况处理
INSERT IGNORE INTO training_courses (id, title, description, duration_minutes, video_url, content, display_order, is_active)
VALUES (3, '紧急情况处理', '掌握紧急情况的应对方法和求助流程', 10, NULL, '<h2>应急预案</h2><p>1. 独立SOS求助流程</p><p>2. 求助流程</p><p>3. 后续处理</p>', 3, true);
